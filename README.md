...
### Щоденний новий «мозок» без забування помилок

- Щоночі моделі **донавчаються** (warm-start) від попереднього brain — не обнуляються
- Усі ринкові приклади + уроки з угод накопичуються в `data/experience/` (**ніколи не видаляються** при ротації)
- Збиткові угоди запам’ятовуються сильніше (lesson memory)
- `MODEL_KEEP_VERSIONS` чистить лише зайві **копії файлів** на диску, не пам’ять і не досвід
- Угоди/ризик у `data/trading.db` — після рестарту ПК бот **продовжує**
- Telegram при старті: «продовжив роботу» + версія brain
